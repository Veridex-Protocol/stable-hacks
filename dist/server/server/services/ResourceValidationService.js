...
const RELAYER_ROOT_CANDIDATES = Array.from(new Set([
...
export class ResourceValidationService {
    ...
    async validateAll() {
        const docsChecks = await Promise.all([
            this.validateUrl('solana-quick-start', 'Solana Quick Start', 'docs', 'https://solana.com/docs/intro/quick-start'),
            this.validateUrl('colosseum-cypherpunk-resources', 'Colosseum Cypherpunk Resources', 'docs', 'https://colosseum.com/cypherpunk/resources'),
            this.validateUrl('circle-faucet', 'Circle Public Faucet', 'api', 'https://faucet.circle.com/'),
    ...
    async validateUrl(id, label, category, target) {
        try {
            const { result, latencyMs } = await timed(async () => fetch(target, {
                method: 'GET',
                redirect: 'follow',
                headers: {
    ...
    async validateUrlWithCurl(id, label, category, target) {
        try {
    ...
    async validateSolanaRpc() {
        ...
        try {
            ...
            return {
                ...
                statusCode: result.status,
                latencyMs,
                details: typeof result.slot === 'number'
