...
function makeReason(
  code: string,
  description: string,
  severity: VerdictReason['severity'],
): VerdictReason {
...
export class PolicyEngine {
  ...
  async evaluate(params: {
    ...
    const checks = {
      ...
      corridorAllowed:
        policy.allowedCorridors.includes(input.corridor) &&
    ...
    if (!checks.assetAllowed) {
      reasons.push(
        makeReason(
          'ASSET_NOT_ALLOWED',
          'The selected SPL stablecoin mint is not approved by the treasury policy.',
          'error',
        ),
      );
    }
    ...
    if (!checks.kycValid) {
      reasons.push(
        makeReason(
          'KYC_REQUIRED',
          'Counterparty KYC is not in a verified state, so settlement must fail closed.',
          'error',
        ),
      );
    }
    ...
    if (!checks.sanctionsClean) {
      reasons.push(
        makeReason(
          'SANCTIONS_BLOCK',
          'The counterparty is marked as sanctioned and cannot receive treasury funds.',
          'error',
        ),
      );
    }
    ...
    if (!checks.corridorAllowed) {
      reasons.push(
        makeReason(
          ...
          'The requested payout corridor is not approved for this policy and counterparty.',
          'error',
        ),
      );
    }
    ...
    if (!checks.amountWithinLimit) {
      reasons.push(
        makeReason(
          'MAX_TX_LIMIT',
          `Requested amount exceeds the single-transfer ceiling of ${policy.maxTransactionAmount}.`,
          'error',
        ),
      );
    }
    ...
    if (!checks.dailyLimitOk) {
      reasons.push(
        makeReason(
          ...
          `Approving this payout would exceed the daily treasury limit of ${policy.dailySendLimit}.`,
          'error',
        ),
      );
    }
    ...
    if (!checks.travelRuleCompliant) {
      reasons.push(
        makeReason(
          'TRAVEL_RULE_MISSING',
          'Travel Rule metadata is required for this amount threshold before settlement can proceed.',
          'error',
        ),
      );
    }
    ...
    if (telemetry.riskLevel === 'critical') {
      reasons.push(
        makeReason(
          'KYT_CRITICAL',
          'Live Solana telemetry flagged the recipient as critical risk based on address history.',
          'error',
        ),
      );
      ...
      reasons.push(
        makeReason(
          'KYT_HIGH',
          'Live Solana telemetry flagged elevated recipient risk, so manual approval is required.',
          'warning',
        ),
      );
      ...
      reasons.push(
        makeReason(
          'KYT_MEDIUM',
          'Recipient telemetry is thin enough that a finance approver should review it.',
          'warning',
        ),
      );
    } else {
      reasons.push(
        makeReason(
          'KYT_OK',
          `Live Solana telemetry returned a ${telemetry.riskLevel}-risk profile for the recipient.`,
          'info',
        ),
      );
    }
    ...
    if (input.amount >= policy.escalationThreshold) {
      reasons.push(
        makeReason(
          'VALUE_ESCALATION',
          `Amount is at or above the escalation threshold of ${policy.escalationThreshold}.`,
          'warning',
        ),
      );
    }
    ...
    const hasEscalationSignal =
      reasons.some((reason) => reason.severity === 'warning') || input.amount >= policy.escalationThreshold;
